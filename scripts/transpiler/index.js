...
/**
 * @param {import("../parser/ast-types.js").AstNode} node
 * @returns {string}
 */
export function convertNode(node) {
    switch (node.type) {
        case "import":
            return generateRequire(node);
        case "element":
            return generateReactCreateElement(node);
        case "expression":
            return node.children.map(convertNode).join("");
        case "text":
            return node.value;
        case "export-default":
            return generateExportDefault();
        case "export-function":
            return generateExportFunction(node);
        case "export-class":
            return generateExportClass(node);
        case "export-declarators":
            return generateExportDeclarators(node);
        case "export-named":
            return generateExportNamed(node);
        case "export-all":
            return generateExportAll(node);
        default:
            return "";
    }
}
